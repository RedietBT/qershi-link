...
public class Sacco {
    ...
    public Sacco(UUID saccoId, String saccoName, String schemaName, boolean isUnion, BigDecimal minShareRequirement) {
        this.saccoId = saccoId != null ? saccoId : UUID.randomUUID();
        this.saccoName = saccoName;
        this.schemaName = schemaName;
        this.isUnion = isUnion;
        this.minShareRequirement = minShareRequirement != null ? minShareRequirement : BigDecimal.ZERO;
        this.status = SaccoStatus.PENDING_SETUP;
        this.createdAt = Instant.now();
        this.updatedAt = Instant.now();
    }

    // Business Rule: Ensure a SACCO cannot be its own parent Union
    public void attachToUnion(UUID unionId) {
        if (this.saccoId.equals(unionId)) {
            throw new IllegalArgumentException("A SACCO cannot be its own parent Union.");
        }
