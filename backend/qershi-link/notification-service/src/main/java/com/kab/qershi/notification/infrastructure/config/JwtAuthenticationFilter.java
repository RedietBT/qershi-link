...
/**
 * Filter for intercepting incoming HTTP requests, validating JWT access tokens,
 * extracting authorities, and dynamically resolving tenant schema into TenantContext.
 *
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
                    ...
                    if (saccoSchema != null && !saccoSchema.isBlank()) {
                        ...
                        if (saccoIdStr != null && !saccoIdStr.isBlank()) {
                            try {
                                ...
                                log.warn("Failed to resolve tenant schema for saccoId {}: {}", saccoIdStr, ex.getMessage());
                            }
                        }
                    }
                }
    ...
    private Claims parseClaims(String token) {
        try {
            ...
            log.warn("Failed to parse JWT token in notification-service: {}", ex.getMessage());
            return null;
        }
    }
    ...
    private String lookupSaccoSchema(UUID saccoId) {
        try (Connection conn = dataSource.getConnection();
            ...
            log.error("Failed querying sacco_registry for saccoId {}: {}", saccoId, ex.getMessage());
        }
        return null;
    }
}
