 ...
 * Dispatches automated loan origination SMS notifications (e.g. LOAN_APPLICATION_APPROVED).
 *
...
public class NotificationGrpcClientAdapter {
    ...
    public void sendNotification(String recipientPhone, String templateCode, Map<String, String> parameters) {
        if (recipientPhone == null || recipientPhone.isBlank()) {
            log.warn("Cannot dispatch loan origination SMS: Recipient phone is null or blank.");
            return;
        }

        try {
            String tenantSchema = TenantContext.getTenantSchema();
            log.info("Sending loan origination SMS [Template: {}] to {} via gRPC notification-service for schema: {}", templateCode, recipientPhone, tenantSchema);
            ...
            log.error("Failed to send loan origination SMS via gRPC: {}", ex.getMessage(), ex);
        }
    }
}
