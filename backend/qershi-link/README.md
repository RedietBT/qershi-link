...
$$\text{1. Late Fees \& Penalties} \longrightarrow \text{2. Accrued Interest Due} \longrightarrow \text{3. Outstanding Principal Balance}$$
...
- **Dynamic RBAC & Maker-Checker**: Fine-grained resource action permissions coupled with enterprise administrator roles (`SACCO_ADMIN`, `ADMIN`).
...
docker build -t account-management-service:latest -f account-management-service/Dockerfile .
docker build -t transaction-management-service:latest -f transaction-management-service/Dockerfile .
docker build -t loan-origination-service:latest -f loan-origination-service/Dockerfile .
docker build -t loan-management-service:latest -f loan-management-service/Dockerfile .
